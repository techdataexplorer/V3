#Created by Eric Cutherell
#made adjustments to lines 159,169(try except), 232, 240, 243
...
class Report(object):
    ...
    def __init__(self):
        self.ExampleS3FilePath = ""

    def setFolderPath(self, folderPath):
        self.ExampleS3FilePath = str(folderPath)

    def generateReport(self):

        # Create file paths using main folder path
        PathsFilePath = self.ExampleS3FilePath + "/Paths.csv"
        Pass2FilePath = self.ExampleS3FilePath + "/Data/PathDataPass2.csv"
        Pass3FilePath = self.ExampleS3FilePath + "/Data/PathDataPass3.csv"
        ReportFilePath = self.ExampleS3FilePath + "/Data/Report.txt"
        ...
        for index in range(0, NUMBER_OF_PATHS):
            ...
            if ITURRainOutageSecs == "---":
                RainFlag == 1
            if RainFlag == 0:
                #ITUROutageSecs = ITUROutageSecs + str(ITURRainOutageSecs)
                ITUROutageSecs = float(ITUROutageSecs) + float(ITURRainOutageSecs)
            ...
            try:
                ITURavailability = 100 * (1 - (float(ITUROutageSecs) / Tsyear))
            except ValueError:
                #find where character is
                counter = 0
                for character in ITUROutageSecs:
                    if character.isalpha() == True:
                        break
                    counter = counter + 1
                #truncate it
                truncatedITUROutageSecs = ITUROutageSecs[0:counter]
                #do calculations
                ITURavailability = 100 * (1 - (float(truncatedITUROutageSecs) / Tsyear))
            ...
            report.write("Annual two-way Dispersive Multipath Outage Seconds: " + str(VBTotalDFSecs) + "\n")
            ...
            report.write("Annual two-way Flat Fading Multipath Outage Seconds: " + str(ITURTotalFFSecs) + "\n")

            #report.write("Annual two-way Dispersive Multipath Outage Seconds: " + str(round(ITURTotalDFSecs, 1)) + "\n")
            report.write("Annual two-way Dispersive Multipath Outage Seconds: " + str(ITURTotalDFSecs) + "\n")
...
###"C:/Users/ecuth/Desktop/Spatial Datalyst/Kizer/Path Design 11 April 2021/Step 3 Path Availability/ExampleS3"
